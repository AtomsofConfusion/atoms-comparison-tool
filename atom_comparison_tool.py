...
for filename in os.listdir(codeQL_directory):
    if filename.endswith(".sarif"):
        ...
        p = pd.DataFrame(
            columns=["Type", "File", "Line", "CQL", "CQL_Column", "CQL_CodeQL"]
        )
        rows = []
    ...
    for i in range(len(json_value["runs"][0]["results"])):
        s = pd.Series(
            {
                "Type": filename[:-6],
                "File": json_value["runs"][0]["results"][i]["locations"][0][
                    "physicalLocation"
                ]["artifactLocation"]["uri"],
                "Line": json_value["runs"][0]["results"][i]["locations"][0][
                    "physicalLocation"
                ]["region"]["startLine"],
                "CQL": "+",
                "CQL_Column": json_value["runs"][0]["results"][i]["locations"][0][
                    "physicalLocation"
                ]["region"].get("startColumn", None),
                "CQL_Code": json_value["runs"][0]["results"][i]["locations"][0][
                    "physicalLocation"
                ]["contextRegion"]["snippet"]["text"].splitlines()[2:-2],
            }
        )
        rows.append(s)
...
clojure_master.rename(
    columns={"atom": "Type", "file": "File", "line": "Line", "code": "CLJ_Code"},
    inplace=True,
)
...
for key, df in dataframes.items():
    k = pd.merge(
        clojure_master[clojure_master["Type"] == key],
        df,
        on=["File", "Type", "Line"],
        how="outer",
    )
    ...
    k["CQL"] = k["CQL"].fillna("-")
    dataframes[key] = (
        k.sort_values(by=["File", "Line"]).reset_index().drop("index", axis=1)
    )
...
for i in coccinelle_dataframes.values():
    ...
    k = k.drop_duplicates(
        subset=["Type", "File", "Line", "CLJ", "CQL", "CNL"], keep="first"
    )
